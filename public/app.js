...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Jika role admin atau superadmin, tampilkan rekap absen
...
// Fungsi Ambil dan Tampilkan Data Absensi
async function loadAttendance() {
    try {
        ...
        data.forEach(item => {
            ...
            let actionButtons = '';
            // KONTROL HAK AKSES: Tombol Edit & Hapus HANYA muncul untuk 'superadmin'
            ...
            // Tampilkan tanggal beserta jam/waktu jika ada
            const infoWaktu = item.waktu ? `${item.created_at} - Pukul ${item.waktu}` : item.created_at;

            li.innerHTML = `
                <span><b>${item.name}</b> — Status: <b>${item.status}</b> <i style="color: gray; font-size: 0.9em;">(${infoWaktu})</i></span>
                ${actionButtons}
            `;
            list.appendChild(li);
        });
...
// Fungsi Edit Absen (Hanya bisa dijalankan jika superadmin memanggilnya)
...
// Fungsi Hapus Absen (Hanya bisa dijalankan jika superadmin memanggilnya)
